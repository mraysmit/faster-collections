...
public class AsynchronousExecutorTest {
    ...
    private static final Integer NUM_OF_ELEMENTS_TEST = 1000000;
    private static final int NUM_OF_THREADS = 8;

    private static final int NUM_OF_REPETITIONS = 5;
    private static final int STARTING_REPETITION = 1;
    ...
    private void memorizeTestInfo(int repetition, String testName) {
        ...
        if (currentRepetion == STARTING_REPETITION) {
            timeMemory = new Long[NUM_OF_REPETITIONS];
            threadsTimeMemory = new Long[NUM_SET_THREADS][NUM_OF_REPETITIONS];
    ...
    void computeCPUTime(Long time) {
        ...
        if (currentRepetion == NUM_OF_REPETITIONS) {
    ...
    void computeThreadCPUTime(Long time, int threadSet, int repetition) {
        ...
        if (threadSet == START_SET_THREADS && repetition == STARTING_REPETITION) {
            threadsTimeMemory = new Long[NUM_SET_THREADS][NUM_OF_REPETITIONS];
        }

        threadsTimeMemory[threadSet - 1][repetition - 1] = time;

        if (currentThreadSet == NUM_SET_THREADS && repetition == NUM_OF_REPETITIONS) {

            double[] averages = new double[NUM_OF_THREADS];
    ...
    @Order(1)
    @RepeatedTest(value = NUM_OF_REPETITIONS)
    public void testStream(RepetitionInfo info) {
        this.memorizeTestInfo(info.getCurrentRepetition(), "testStream");
        var asyncExec = new AsynchronousExecutor<String, String>(NUM_OF_THREADS);
        var transformer = new UpperCaseConverter();

        List<String> inputList = this.generator.objects(String.class, NUM_OF_ELEMENTS_TEST).collect(Collectors.toList());

        Instant start = Instant.now();
        asyncExec.processStream(inputList, transformer);
    ...
    public void testParallelStream(RepetitionInfo info) {
        ...
        Instant start = Instant.now();
        asyncExec.processParallelStream(inputList, transformer);
        Instant end = Instant.now();
        this.computeCPUTime(Duration.between(start, end).toMillis());
    ...
    public void testSublistPartition(RepetitionInfo info) {
        ...
        var asyncExec = new AsynchronousExecutor<String, String>(NUM_OF_THREADS);
        var transformer = new CollectionUpperCaseConverter();

        List<String> inputList = this.generator.objects(String.class, NUM_OF_ELEMENTS_TEST).collect(Collectors.toList());

        Instant start = Instant.now();
        asyncExec.processSublistPartition(inputList, transformer);
        asyncExec.shutdown();
        Instant end = Instant.now();
        this.computeCPUTime(Duration.between(start, end).toMillis());
    ...
    public void testShallowPartitionList(RepetitionInfo info) {
        ...
        List<String> inputList = this.generator.objects(String.class, NUM_OF_ELEMENTS_TEST).collect(Collectors.toList());
        var expectedList = inputList.stream().map(e -> transformer.apply(e)).collect(Collectors.toList());

        Instant start = Instant.now();
        asyncExec.processShallowPartitionList(inputList, transformer);
        asyncExec.shutdown();
        Instant end = Instant.now();
        this.computeCPUTime(Duration.between(start, end).toMillis());
    ...
    public void testShallowPartitionArray(RepetitionInfo info) {
        ...
        asyncExec.processShallowPartitionArray(inputList, transformer);
        asyncExec.shutdown();
        var end = Instant.now();
        this.computeCPUTime(Duration.between(start, end).toMillis());
    ...
    public void speedUpShallowPartitionArray(int numThreads) {
        ...
        IntStream.rangeClosed(1, NUM_OF_REPETITIONS).forEach(repetition -> {
